...
export async function updateEmbeddings(
  ...
  if (updates.length > 0 || formattingUpdates.length > 0) {
    ...
    for (const { mdn_url, title, hash, html, text } of updates) {
      try {
    ...
    for (const { mdn_url, title, html } of formattingUpdates) {
      try {
        console.log(
...
async function fetchAllExistingDocs(supabase: SupabaseClient) {
  ...
  const selectDocs = () =>
    supabase
      .from("mdn_doc_macro")
      .select("id, mdn_url, title, hash, token_count")
      .order("id")
      .limit(PAGE_SIZE);
